...
# --- PATCH START (search for "class BetService" and modify place_bet) ---
...
class BetService:
    ...
    def place_bet(session, user: User, game_key: str, amount: Decimal, params: Dict[str,Any]) -> Bet:
        ...
        # FIX: do NOT call asyncio.run() inside an already running loop
        try:
            running_loop = asyncio.get_running_loop()
        except RuntimeError:
            # No loop running (pure sync context)
            result: BetResult = asyncio.run(async_play())
        else:
            # We are inside an event loop (telegram handlers). Use run_coroutine_threadsafe
            fut = asyncio.run_coroutine_threadsafe(async_play(), running_loop)
            result: BetResult = fut.result()
...
# --- PATCH START (replace main() function near end of file) ---

def main():
    if Config.BOT_TOKEN == "8396606980:AAE3-QlN2wRq83jNgqKoUqM8v6PgrMUTR98":
        logger.error("Set BOT_TOKEN env var before running.")
        return
    init_db()
    app = build_application()

    async def after_startup(app_: Application):
        # Schedule background tasks AFTER bot started
    ...
    try:
        ...
        logger.exception("Fatal error caused shutdown")
